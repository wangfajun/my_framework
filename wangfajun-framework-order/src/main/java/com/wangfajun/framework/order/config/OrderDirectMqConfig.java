...
public class OrderDirectMqConfig {
	...
	public Queue directOrderQueue() {
		...
		return new Queue(OrderQueueConstants.DIRECT_ORDER_QUEUE,true,  false,  false, arguments);
	}
	...
	public DirectExchange directTxEventExchange() {
		return new DirectExchange(RabbitMqCommonConstants.DIRECT_TX_EVENT_EXCHANGE,  true,  false);
	}

	/**
	 * 订单队列、公共事件(直接交换机)、公共事件路由key 绑定
	 *
	 * @return
	 */
	@Bean
	public Binding bindDirectTxEventExchangeAndDirectOrderQueue() {
		return BindingBuilder.bind(directOrderQueue()).to(directTxEventExchange()).with(RabbitMqCommonConstants.DIRECT_TX_EVENT_KEY);
	}
