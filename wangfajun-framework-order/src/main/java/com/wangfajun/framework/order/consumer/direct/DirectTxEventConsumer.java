package com.wangfajun.framework.order.consumer.direct;
...
public class DirectTxEventConsumer {
	...
	@RabbitHandler
	@RabbitListener(queues = OrderQueueConstants.DIRECT_ORDER_QUEUE)
	public void consumeTxEvent(Message message,Channel channel, @Headers Map<String,Object> headers) throws Exception {
		...
		try {
			...
			Object retryCountObj = redisUtil.get(OrderQueueConstants.DIRECT_ORDER_QUEUE + txEvent.getId());
			...
			if (ObjectUtil.isNotEmpty(retryCountObj) && (int)retryCountObj >= 3) {
				...
				redisUtil.incr(OrderQueueConstants.DIRECT_ORDER_QUEUE + txEvent.getId(), 1);
				log.info("------ 放入消费队列重试消费");
			}
		}
	}
