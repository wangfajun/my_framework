...
@Slf4j
@Component
public class PayEventDeadConsumer {
	 ...
	 * @param headers
	 */
	@RabbitListener(queues = {CommonConstants.ORDER_DEAD_QUEUE})
	@RabbitHandler
	public void consumeDead(Message message, Channel channel, @Headers Map<String,Object> headers) throws Exception {
		...
		try {
			log.info("订单死信队列消费端收到消息:"+message);
			// 应答模式确认接收到消息,后面那个参数如果是false,就是只确认签收当前消息,如果是true,则签收全部比当前TagId小的消息
			channel.basicAck(deliveryTag,  false);

			// 清除key
			redisUtil.del(String.valueOf(payEvent.getId()));
		}catch (Exception e){
			log.error("死信队列消费事件异常",e);
			try {
