 ...
 * 积分服务 积分队列 扇形交换机 配置
 *
 * @description: 积分服务与订单服务做分布式事务（基于本地事件表）
 * 积分回调成功后，向积分服务本地事务表中插入事件数据，待定时任务拉取后，向MQ推，订单服务（order）消费消息，做订单状态处理
 * 
 * RabbitMq中4中交换机：
 ...
 */
@Configuration
public class PointFanoutMqConfig {
	...
	public Queue fanoutPointQueue() {
		...
		return new Queue(PointQueueConstants.FANOUT_POINT_QUEUE,true,  false,  false, arguments);
	}
	...
	public FanoutExchange fanoutTxEventExchange() {
		return new FanoutExchange(RabbitMqCommonConstants.FANOUT_TX_EVENT_EXCHANGE);
	}
	 ...
	 */
	@Bean
	public Binding bindFanoutTxEventExchangeAndPointQueue() {
		return BindingBuilder.bind(fanoutPointQueue()).to(fanoutTxEventExchange());
	}
