 ...
 */
@Configuration
public class PointDirectMqConfig {
	 ...
	 */
	@Bean
	public Queue directPointQueue() {
		Map arguments =  new HashMap<>();
		arguments.put("x-dead-letter-exchange", PointDeadQueueConstants.POINT_DEAD_EXCHANGE);
		arguments.put("x-dead-letter-routing-key", PointDeadQueueConstants.POINT_DEAD_KEY);
		return new Queue(PointQueueConstants.DIRECT_POINT_QUEUE,true,  false,  false, arguments);
	}
	 ...
	 */
	@Bean
	public DirectExchange directTxEventExchange() {
		return new DirectExchange(RabbitMqCommonConstants.DIRECT_TX_EVENT_EXCHANGE);
	}

	/**
	 * 公共事件(直接交换机)、支付队列(直接交换机) 绑定
	 *
	 * @return
	 */
	@Bean
	public Binding bindDirectTxEventExchangeAndDirectPointQueue() {
		return BindingBuilder.bind(directPointQueue()).to(directTxEventExchange()).with(RabbitMqCommonConstants.DIRECT_TX_EVENT_KEY);
	}
