package com.wangfajun.framework.point.consumer.fanout;
...
public class FanoutTxEventConsumer {
	...
	public void consumeTxEvent(Message message,Channel channel, @Headers Map<String,Object> headers) throws Exception {
		...
		try {
			...
			Object retryCountObj = redisUtil.get(PointQueueConstants.FANOUT_POINT_QUEUE + txEvent.getId());
			...
			if (ObjectUtil.isNotEmpty(retryCountObj) && (int)retryCountObj >= 3) {
				...
				redisUtil.incr(PointQueueConstants.FANOUT_POINT_QUEUE + txEvent.getId(), 1);
